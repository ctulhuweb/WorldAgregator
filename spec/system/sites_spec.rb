...
RSpec.describe "Sites managemet", type: :system, js: true do
  ...
  describe "settings" do
    ...
    it "have card info", do
      site = create(:site, aggregator: @ag)
      dec = SiteDecorator.new(site)
      visit aggregator_sites_path(@ag)
      click_link "Setting"
      expect(page).to have_content(dec.url_info)
      expect(page).to have_content(dec.status_info)
      expect(page).to have_selector("input[value='#{dec.active_btn_title}']")
      expect(page).to have_selector(".btn-#{dec.active_btn_style}")
    end
